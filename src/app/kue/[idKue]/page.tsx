...
async function getDetailKue(id: string): Promise<Kue | null> {
  ...
  const response = await fetch(`${baseUrl}/api/kue/${id}`, {
    cache: 'no-store' // Atau 'no-store', atau atur di segment config
    // next: { revalidate: 60 },
  });
...
export default async function HalamanDetailKue({ params: paramsPromise }: {params : Promise <{idKue: string }>}) {
